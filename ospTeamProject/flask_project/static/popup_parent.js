...
document.getElementById("allergy").onclick = function(){
    ...
    if(document.getElementsByClassName("allergylist").value!=""){
        popup.addEventListener('unload', function () {
            document.getElementsByClassName("allergy").checked = true;
    })}
    //allergylist에 아무것도 들어있지 않을 때 팝업창을 닫으면 menuUpload.html의 allergy 체크박스 선택이 해제되도록 설정
    else{
        popup.addEventListener('unload', function () {
            document.getElementsByClassName("allergy").checked = false;
    })
    }
